...
int post_request(int client_sd, char *body, short content_length)
{
	...
	sprintf(buf2, "{\"" KEY_ID "\":%d,\"" KEY_TITLE "\":\"%s\",\"" KEY_DESCRIPTION "\":\"%s\"}", ids - 1, title, description);
	sprintf(buf1, RESPONSE_201 CRLF CONTENT_LENGTH ": %lu" CRLF CONTENT_TYPE ": " JSON_TYPE CRLF CRLF "%s", strlen(buf2), buf2);
...
int get_request(int client_sd, short todo_id)
{
	...
	if (todo_id != -1)
	{
		...
		sprintf(buf2 + strlen(buf2),
				"{\"" KEY_ID "\":%d,\"" KEY_TITLE "\":\"%s\",\"" KEY_DESCRIPTION "\":\"%s\"}", todo->id, todo->title,
				todo->description);
	}
	else
	{
		...
		for (todo = todo_root; todo; todo = todo->next)
			sprintf(buf2 + strlen(buf2),
					"{\"" KEY_ID "\":%d,\"" KEY_TITLE "\":\"%s\",\"" KEY_DESCRIPTION "\":\"%s\"}%s", todo->id, todo->title,
					todo->description, todo->next ? "," : "");
		*(buf2 + strlen(buf2)) = ']';
	}
	sprintf(buf1, RESPONSE_200_NOCRLF CRLF CONTENT_LENGTH ": %lu" CRLF CONTENT_TYPE ": " JSON_TYPE CRLF CRLF "%s", strlen(buf2), buf2);
	return (send_response(client_sd, buf1));
}
